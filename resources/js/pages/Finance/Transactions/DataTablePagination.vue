...
watch(selectedPerPage, (newValue) => {
    ...
    const currentDateFrom = new URLSearchParams(window.location.search).get('date_from');
    if (currentDateFrom) {
        url.searchParams.set('date_from', currentDateFrom);
    }

    const currentDateTo = new URLSearchParams(window.location.search).get('date_to');
    if (currentDateTo) {
        url.searchParams.set('date_to', currentDateTo);
    }

    const currentTransactionTypeId = new URLSearchParams(window.location.search).get('transaction_type_id');
    if (currentTransactionTypeId) {
        url.searchParams.set('transaction_type_id', currentTransactionTypeId);
    }
